                ...
                <div class="ui stackable two column relaxed grid container" style="padding: 20px">
                    <div class="column" style="padding: 0">
                        ...
                        <div class="row border-top-paperviu-sky" style="background-color: #f1f5f9; margin-top: 20px">
                            <div class="ui row" style="padding: 16px">
                                <h3 class="ui header text-paperviu-sky"><i
                                        class="user outline circle text-paperviu-d7 icon"></i>{{ contenidoActual.empresa
                                    || '' }}</h3>
                            </div>
                            <div class="ui row" style="padding: 16px">
                                <h3 class="ui header text-paperviu-sky"><i class="calendar text-paperviu-d7 icon"></i>{{
                                    contenidoActual.fecha || 'Desconocido...' }}
                                </h3>
                            </div>
                        </div>
                        <div class="row" style="margin-top: 20px">
                            <template v-for="atributo in contenidoActual.atributos">
                                <div class="row border-left-paperviu-sky"
                                    ...
                                    <div class="ui ribbon label header inverted paperviu-sky" style="padding-top: 5px; padding-bottom: 5px">
                                        {{ atributo.clave}}
                                    </div>
                                    <h3 class="ui header text-paperviu-dark"
                    ...
                    <div class="column" style="padding-top: 0;">
                        <div class="row border-top-paperviu-sky paperviu-e8">
                            <h2 class="ui text-paperviu-sky header" style="margin: 0; padding: 10px"><i
                                    class="comments icon text-paperviu-d7"></i>Comentarios</h2>
                            <div id="container" v-bar>
                                <div class="ui large comments"
                                     style="margin: 0; padding-left: 20px; padding-right: 20px; height: 400px; max-height: 400px">
                                    <template v-for="comentario in contenidoActual.comentarios">
                                        <div class="comment paperviu-e4"
                                             style="margin-top: 16px">
                                            <div class="content">
                                                <h3 v-if="comentario.spoiler >= 5"
                                                    class="ui author header small text-paperviu-sky inverted"
                                                    style="border-left:solid; padding-left: 7px; margin: 0">
                                                    Spoiler!
                                                </h3>
                                                <span class="ui author header small text-paperviu-d7 border-left-paperviu-sky"
                                                      style="border-left:solid; padding-left: 7px">
                                                        {{ comentario.username}}
                                                </span>
                                                <div class="metadata">
                                                    <h3 class="ui header text-paperviu-dark small date"
                                                        style="font-weight: 500">
                                                        {{ comentario.fecha + " " + comentario.hora || ''}}</h3>
                                                </div>
                                                <div v-if="comentario.spoiler >= 5 && !comentario.mostrarSpoiler"
                                                     class="row" style="padding: 7px">
                                                    <button @click="comentario.mostrarSpoiler = true"
                                                            class="ui compact basic button paperviu-d7 hover-paperviu-dark text-paperviu-e8">
                                                        <i class="unhide icon text-paperviu-sky"></i>Mostrar spoiler
                                                    </button>
                                                </div>
                                                <div v-show="comentario.spoiler < 5 || comentario.mostrarSpoiler"
                                                     class="ui header small text-paperviu-dark text"
                                                     style="padding-left: 10px; font-weight: 500; padding-bottom: 5px; margin-bottom: 0">
                                                    {{comentario.texto}}
                                                </div>
                                                <div style="padding-left: 10px; padding-bottom: 5px; padding-top: 0"
                                                     v-if="comentario.username !== usuario.username && comentario.spoiler < 5"
                                                     class="actions">
                                                    <a @click="idComentarioActual = comentario.id; setComentarioSpoiler();"
                                                       class="reply"
                                                       style="font-size: 15px">Spoiler!</a>
                                                </div>
                                            </div>
                                        </div>
                                    </template>
...
<script>
    ...
    export default {
        ...
        computed: {
            ...
            setComentarioSpoilerURL() {
                return this.$store.state.baseUrl + "comentarios?id=" + this.idComentarioActual;
            }
        },
        methods: {
            toggleVideosSidebar() {
            ...
            getComentarios($state) {
                var _this = this;
                setTimeout(() => {
                    $.get(_this.getComentariosURL, function (response) {
                            ...
                            if (response.length >= 1) {
                                ...
                                $.each(response, function (index, comentario) {
                                    ...
                                    comentarios.push({
                                        ...
                                        spoiler: Number(comentario.spoiler || 0),
                                        mostrarSpoiler: false
                                    });
                                });
                                _this.contenidoActual.comentarios = _this.contenidoActual.comentarios.concat(comentarios);
                                $state.loaded();
                                _this.pagina += 1;
                            } else $state.complete();
                        } else $state.complete();
            ...
            setComentarioSpoiler() {
                if (this.idComentarioActual !== '') {
                    var _this = this;
                    _this.mostrarInfo("Marcando como spoiler...");
                    $.ajax({
                        type: 'PUT',
                        url: _this.setComentarioSpoilerURL,
                        success: function (response) {
                            _this.mostrarExito("Marcado como spoiler!");
                        },
                        error: function () {
            ...
            cargarComentarios() {
                this.pagina = 1;
                this.contenidoActual.comentarios = [];
        ...
        watch: {
            ...
            idContenido(newValue, oldValue) {
                if (newValue !== '' && newValue !== oldValue) this.cargarComentarios();
            }
        },
        components: {
            InfiniteLoading
